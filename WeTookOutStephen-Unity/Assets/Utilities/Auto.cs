...
public static class Auto
{
    ...
    public static IEnumerator Anim(System.Action<float> setter, float start, float end, float duration, Easer ease)
    {
        float elapsed = 0;
        //var range = alphaTarget - start;
        setter(start);
        while (elapsed < duration)
        {
            elapsed = Mathf.MoveTowards(elapsed, duration, Time.deltaTime);
            //transform.localPosition = start + range * ease(elapsed / duration);
            setter(Mathf.Lerp(start, end, ease(elapsed / duration)));
            yield return 0;
        }
        setter(end);
    }

    public static IEnumerator Anim<T>(System.Action<T> setter, System.Func<T, T, float, T> lerp, T start, T end, float duration, Easer ease)
    {
        float elapsed = 0;
        //var range = alphaTarget - start;
        setter(start);
        while (elapsed < duration)
        {
            elapsed = Mathf.MoveTowards(elapsed, duration, Time.deltaTime);
            //transform.localPosition = start + range * ease(elapsed / duration);
            setter(lerp(start, end, ease(elapsed / duration)));
            yield return 0;
        }
        setter(end);
    }

    public static IEnumerator FadeTo(this Renderer r, float alphaTarget, float duration, Easer ease)
    {
        float elapsed = 0;
        Material mat = r.material;
        var start = mat.color;
        var end = new Color(start.r, start.g, start.b, alphaTarget);
        while (elapsed < duration)
        {
            elapsed = Mathf.MoveTowards(elapsed, duration, Time.deltaTime);
            mat.color = Color.Lerp(start, end, ease(elapsed / duration));
            yield return 0;
        }
        mat.color = end;
    }

    public static IEnumerator Fade(this Renderer r, Color start, Color end, float duration, Easer ease)
    {
        float elapsed = 0;
        Material mat = r.material;
        mat.color = start;
    ...
    //public static IEnumerator FadeTo(this TextMesh tm, float alphaTarget, float duration, Easer ease)
    //{
    //    // Animates the text mesh's default color. Colored rich text is not affected by this.
    //    // Use FadeTo(this Renderer) to affect rich text as well.
        
    //    float elapsed = 0;
    //    Color start = tm.color;
    //    Color end = start.ReplaceAlpha(alphaTarget);
    //    while (elapsed < duration)
    //    {
    //        elapsed = Mathf.MoveTowards(elapsed, duration, Time.deltaTime);
    //        tm.color = Color.Lerp(start, end, ease(elapsed / duration));
    //        yield return 0;
    //    }
    //    tm.color = end;
    //}

    public static IEnumerator Fade(this TextMesh tm, Color start, Color end, float duration, Easer ease)
    {
        // Animates the text mesh's default color. Colored rich text is not affected by this.
        // Use Fade(this Renderer) to affect rich text as well.
    ...
    public static IEnumerator FadeTo(this SpriteRenderer s, float alphaTarget, float duration, Easer ease)
    {
        float elapsed = 0;
        var start = s.color;
        var end = new Color(s.color.r, s.color.g, s.color.g, alphaTarget);
        //var range = alphaTarget - start;
        while (elapsed < duration)
        {
            elapsed = Mathf.MoveTowards(elapsed, duration, Time.deltaTime);
            //transform.localPosition = start + range * ease(elapsed / duration);
            s.color = Color.Lerp(start, end, ease(elapsed / duration));
            yield return 0;
        }
        s.color = end;
    }

    public static IEnumerator Fade(this SpriteRenderer s, Color start, Color end, float duration, Easer ease)
    {
        float elapsed = 0;
        //var range = alphaTarget - start;
        s.color = start;
    ...
    public static IEnumerator ResizeTo(this Camera camera, float size, float duration, Easer ease)
    {
        ...
        return Anim(f => camera.orthographicSize = f, startSize, size, duration, ease);
    }
    ...
    ///// <summary>
    ///// Starts a coroutine that starts a bunch of nested coroutines and waits for all to have completed.
    ///// </summary>
    ///// <param name="owner"></param>
    ///// <param name="coroutines"></param>
    ///// <returns></returns>
    //public static IEnumerator WaitAll(this MonoBehaviour owner, System.Collections.Generic.IEnumerable<IEnumerator> coroutines)
    //{
    //    CoroutineJoin cj = new CoroutineJoin(owner);

    //    foreach (IEnumerator cr in coroutines)
    //    {
    //        cj.StartSubCoroutine(cr);
    //    }

    //    yield return cj.WaitAll();
    //}
...
public static class Ease
{
	...
	public static readonly Easer ExpoOut = (t) => { return 1 - ExpoIn(1 - t); };
